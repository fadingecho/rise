...
void HyperGraph::init(unsigned int n)
{
        sfmt_init_gen_rand(&sfmtSeed, rand());
        node_edge = vector<vector<int>>(n + 1);
        node_edge_map = vector<bool>(n + 1, false);
        node_edge_map_vec.reserve(100000);
        maxDegree = 0;
        numNodes = n;
        curEdge = 0;
}
...
void HyperGraph::updateDeg()
{
        ...
        for (unsigned int i = curEdge; i < num; ++i)
        {
                ...
                for (unsigned int j = 0; j < num2; ++j)
                {
                        ...
                        if(!node_edge_map[edge_node[i][j]])
                        {
                                node_edge_map[edge_node[i][j]] = true;
                                node_edge_map_vec.push_back(edge_node[i][j]);
                        }
                }
        }
...
void HyperGraph::addEdge(vector<int> &edge)
{
        ...
        for (unsigned int i = 0; i < edge.size(); ++i)
        {
                ...
                if(!node_edge_map[edge[i]])
                {
                        node_edge_map[edge[i]] = true;
                        node_edge_map_vec.push_back(edge[i]);
                }
        }
...
/*
 * get the number of hyperedges incident to node n
 */
int HyperGraph::getNumEdgeNode(int n)
{
        return node_edge[n].size();
}
/*
 * get the number of hyperedges
 */
int HyperGraph::getNumEdge()
{
...
bool HyperGraph::testNodeMap(int n)
{
        return node_edge_map[n];
}

const vector<int> &HyperGraph::getNodeMapVec() const
{
        return node_edge_map_vec;
}
...
{
        /*DYH : I modified the clean function for better memory management(hope so)*/
        // hg-pool-like stuff should be impl here
        for (size_t i = 0; i < edge_node.size(); i++)
        {
                vector<int>().swap(edge_node[i]);
        }
        edge_node.clear();

        for (size_t i = 0; i < node_edge.size(); i++)
        {
                node_edge[i].clear();
        }
        for (size_t i = 0; i < node_edge_map_vec.size(); i++)
        {
                node_edge_map[node_edge_map_vec[i]] = false;
        }
        node_edge_map_vec.clear();
...
float getCurrentMemoryUsage()
{
        ...
        string command = "pmap " + pid + " | grep -i -w Total | awk '{print $2}' > " + outfile;
